...
namespace Data.Web.RateMyCoopJob
{
    public static class RateMyCoopJob
    {
        ...
        public static IEnumerable<JobReview> GetListofJobs()
        {
            ...
            var listOfJobs = body.Where(x => x.Name.Equals("tr")).Select(node => new JobReview
            {
        ...
        {
            var url = "http://www.ratemycoopjob.com/job/" + job.JobReviewId;
            ...
            var body = htmlDocument.DocumentNode.SelectSingleNode("//*[@id=\"job_rating_list\"]").ChildNodes;
            

            var ratingsList = body.Where(x => x.Name.Equals("div"));

            foreach (var row in ratingsList)
            {
                var firstOrDefault = row.ChildNodes[1].ChildNodes[1].ChildNodes[1].Attributes
